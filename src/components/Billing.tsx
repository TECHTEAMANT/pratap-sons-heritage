    ...
    try {
      ...
      if (bookings && bookings.length > 0) {
        for (const booking of bookings) {
          if (items.find(i => i.barcode_8digit === booking.barcode_8digit)) {
            continue;
          }
    ...
    try {
      ...
      await supabase
        ...
        .update({
          cgst_5: cgst_5_total,
          sgst_5: sgst_5_total,
          cgst_18: cgst_18_total,
          sgst_18: sgst_18_total,
          igst_5: igst_5_total,
          igst_18: igst_18_total,
        })
        .eq('id', invoice.id);
      ...
      for (const item of items) {
        ...
        if (!success) {
          throw new Error(`Failed to update quantity for barcode ${item.barcode_8digit}`);
        }
      }
      ...
      setSuccess(`Invoice ${invoiceNumber} generated successfully! ${amountPending > 0 ? `Pending: ₹${amountPending.toFixed(2)}` : 'Fully Paid'}`);
      setLastGeneratedInvoiceId(invoice.id);
